    ...
    <div className="page-content space-y-8 flex-1 h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-slate-100 tracking-wide">{t('pt.title')}</h1>
      ...
      {/* Patient List */}
      <div className="bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl p-6 min-h-[400px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {patientList.map((p) => (
              <Link
                ...
                className="flex flex-col p-5 rounded-2xl bg-slate-800/20 border border-white/[0.03] hover:bg-slate-800/50 hover:border-cyan-500/30 transition-all group shadow-lg relative overflow-hidden"
              >
                <div className="flex items-start justify-between mb-4 relative z-10">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center text-lg font-bold text-cyan-400 border border-slate-700 shadow-inner">
                      {p.pseudonym?.[0] || '?'}
                    </div>
                    <div>
                      <h3 className="text-base text-slate-200 font-bold group-hover:text-cyan-300 transition-colors">{p.pseudonym}</h3>
                      <p className="text-xs text-slate-500 font-mono mt-0.5">{p.patient_id}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowConfirm(p.patient_id); }}
                      className="p-1.5 rounded-lg bg-slate-800/80 border border-transparent hover:border-red-500/50 hover:bg-red-500/10 hover:text-red-400 text-slate-500 transition-all opacity-0 group-hover:opacity-100"
                      title={t('pt.delete')}
                    >
