...
class TemporalQuery(BaseModel):
    intent: Literal["latest", "earliest", "previous", "before", "after", "between", "trend"]
...
def filter_temporal(evidence_items: list[Any], query: TemporalQuery) -> list[Any]:
    ...
    if query.intent == "before" and query.target_time:
        filtered = [(t, i) for t, i in valid_items if t < query.target_time]
        filtered.sort(key=lambda x: x[0], reverse=True) # closest first
        return [item for _, item in filtered[:query.k]]
        
    if query.intent == "after" and query.target_time:
        filtered = [(t, i) for t, i in valid_items if t > query.target_time]
        return [item for _, item in filtered[:query.k]]
